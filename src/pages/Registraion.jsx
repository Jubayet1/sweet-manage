...
export default function Registraion() {
	const { setRegData, handleRegistraion, regData, handleSigninWithGoogle, handleSigninWithFacebook, handleSigninWithGithub } = useContext(AuthContext)
	...
	return (
		<div className="flex items-center justify-center h-screen">
			<div className="w-full max-w-md p-4 rounded-md shadow sm:p-8 dark:bg-gray-900 dark:text-gray-100">
				<h2 className="mb-3 text-3xl font-semibold text-center">Registraion a new account</h2>
				<p className="text-sm text-center dark:text-gray-400">already have account?
					<Link to={'/signin'} rel="noopener noreferrer" className="focus:underline hover:underline">Log In here</Link>
				</p>
				<div className="my-6 space-y-4">
					...
					<button onClick={handleSigninWithFacebook} aria-label="Login with Twitter" role="button" className="flex items-center justify-center w-full p-4 space-x-4 border rounded-md focus:ring-2 focus:ring-offset-1 border-gray-400 focus:ring-violet-400">
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" className="w-5 h-5 fill-current">
				...
				<form noValidate action="" className="space-y-8 ng-untouched ng-pristine ng-valid">
					...
					<button onClick={handleRegistraion} type="button" className="w-full px-8 py-3 font-semibold rounded-md bg-violet-400 text-gray-900">Sign up</button>
				</form>
			</div>
		</div>
	)
